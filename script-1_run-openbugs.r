...
folder.location <-"W:\\Dropbox\\metabolism\\September_2014"
#-------------------------------------------------------------
...
wd2<- paste(folder.location, "\\input", sep="")
wd3<- paste(folder.location, "\\output", sep="")

dir.create(file.path(wd1, "\\output\\fitting plots"))
wd4<- paste(folder.location, "\\output\\fitting plots", sep="")
...
for (fname in filenames) 
  ...
  # call OpenBUGS and store results in object "metab"
	...
	setwd(wd3)
	write.csv(output.table, "metab_model_results.csv")  # output file name
...
setwd(wd3)
write.csv(output.table, "metab_model_results.csv", row.names=F)  # output file name
